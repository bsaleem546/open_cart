...
{{--                                <th>In Stock</th>--}}
                            ...
                            @foreach($data as $options)
                                <tr>
                                    <td>{{ $options->_attribute->name }}</td>
                                    <td>{{ $options->name }}</td>
{{--                                    <td>{{ $options->quantity }}</td>--}}
{{--                                    <td>{{ $options->price }}</td>--}}
{{--                                    <td>{{ $options->in_stock == 0 ? 'No' : 'Yes' }}</td>--}}
                                    <td>{{ $options->created_at }}</td>
                                    <td><a href="{{ route('options.edit', $options->id) }}"><i class="fas fa-cog"></i></a>
                                        |
                                        <a href="#" onclick="_delete( {{ $options }} )"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </table>
    ...
    <script>
        ...
        function _delete(data) {
            ...
            document.getElementById('delete').addEventListener('click', (e) => {
                $.ajax({
                    type: 'DELETE',
                    url: main_url+'admin/options/'+data.id,
                    contentType: false,
                    processData: false,
