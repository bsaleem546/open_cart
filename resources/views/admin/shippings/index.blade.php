        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-toolbar">
                    <a href="{{ route('shippings.create') }}" class="btn btn-outline-primary">Add New</a>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="example" class="display" style="min-width: 845px">
                        ...
                        <tbody>
                        @foreach($data as $shippings)
                            <tr>
                                <td>{{ $shippings->city }}</td>
                                <td>{{ $shippings->rate }}</td>
                                <td><a href="{{ route('shippings.edit', $shippings->id) }}"><i class="fas fa-cog"></i></a>
                                     |
                                    <a href="#" onclick="_delete( {{ $shippings }} )"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </table>
...
@section('scripts')
    ...
    <script>
        ...
        function _delete(data) {
            $('#username').html(data.city)
            $('#delete_modal').modal('show')
            document.getElementById('delete').addEventListener('click', (e) => {
                $.ajax({
                    type: 'DELETE',
                    url: main_url+'admin/shippings/'+data.id,
                    contentType: false,
                    processData: false,
